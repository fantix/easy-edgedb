...
这一章我们终于离开了德古拉城堡。下面是本章中发生的事情：

> 一艘船从保加利亚（Bulgaria）瓦尔纳市（Varna）出发，驶入黑海。船上有 **船长（captain）、大副（first mate）、二副（second mate）、厨师（cook）** 和 **五名船员（five crew）**。德古拉也在船上，但没有人知道。一到晚上德古拉都会从棺材里出来，于是每天晚上都会有一个人消失。船上的人都感到十分害怕，但也不知道发生了什么或该做些什么。其中一个人说他看到了一个奇怪的身影在甲板上走来走去，但其他人都不相信他。在抵达英格兰（England）惠特比市（Whitby）前的最后一天，船上只剩下船长一个人了——其他人都消失了。船长知道真相了。他将双手绑在方向盘上，这样即使德古拉找到了他，船也能继续直行。第二天，惠特比（Whitby）的人们看见了一艘搁浅的船，一只狼从上面跳下来并跑到了岸上——这是德古拉的狼化身，但人们并不知道。人们发现了死去的船长，他被绑在了方向盘上，手里还拿着一个笔记本，于是人们开始阅读里面的故事。

> 此时，米娜（Mina）和她的朋友露西（Lucy）恰好也在惠特比（Whitby）度假……

## 多重继承（Multiple inheritance）

在德古拉到达惠特比（Whitby）时，让我们来学习一下多重继承（multiple inheritance）。我们已经学习了如何在一个类型上 `extend` 出另一个类型，我们也已经实践了很多次，比如：从 `Person` 扩展出 `NPC`，从 `Place` 扩展出 `City`，等等。多重继承是指同时对多个类型执行此操作。让我们在船员身上做一下尝试。原著里他们并没有名字，所以我们用编号来表示他们。大多数 `Person` 类型不需要数字编号，因此我们将为需要数字编号的类型创建一个名为 `HasNumber` 的抽象类型：
...
此时，我们还需要从 `Person` 类型的 `name` 中删除 `required`。因为现在不是每个 `Person` 类型的对象都会有一个名字，对于有名字的 `Person`，我们相信我们自然会正确输入其对应的名字。当然，我们还会保留 `exclusive`。

现在我们可以运用多重继承为船员创建 `Crewman` 类型。这很简单：只需在要扩展的每个类型之间添加一个逗号：
...
现在我们有了 `Crewman` 并且不再需要赋予他们名字，只需要给他们一个编号，而函数 `count()` 使我们船员的插入变得很容易。我们只需要重复执行五次下面的语句：
...
因此，如果还没有任何 `Crewman` 类型的对象，新插入的第一个船员将获得编号 1，下一个将获得编号 2，依此类推。所以操作五次后，我们可以通过 `SELECT Crewman {number};` 来查看结果了。我们将得到：
...
现在让我们来插入水手的信息，我们只需给他们一个名字并从枚举中选择一个等级：
...
EdgeDB 有一个叫做 [sequence](https://www.edgedb.com/docs/datamodel/scalars/numeric/#type::std::sequence) 的类型对于给一个类型的对象进行编号很有用。这个类型被定义为“int64 的自动递增序列”，因此一个 `int64` 从 1 开始，每次被使用都会加 1。假设我们有一个 `Townsperson` 类型，并尝试对其使用“sequence”。但我们不能像下面这样简单地编写：
...
删除已经插入的项目（对象），对 `sequence` 类型的数字继续加 1 并不产生影响。比如，如果你插入五个 `Townsperson` 对象，它们的数字 `number` 将是从 1 到 5。然后，如果此时将它们全部删除，然后又插入了一个 `Townsperson`，那么这个数字将会继续增加至 6（而不是重新回到 1）。所以对于 `Crewman` 类型的创建，我们还可以有另一种选择。它非常方便，并且没有重复的可能，但是每次插入时数字都会自行加 1。当然，你 _可以_ 通过使用 `UPDATE` 和 `SET` 生成重复的数字（EdgeDB 不会阻止你），但即便如此，当你进行下一次插入时它仍然会跟踪下一个数字。

## 使用 IS 查询多类型（Using IS to query multiple types）

现在我们已经有相当多的类型扩展自 `Person` 类型，其中很多都有自己的属性。`Crewman` 类型有 `number` 属性，同时 `NPC` 类型有一个叫做 `age` 的属性。

如果我们想同时查询它们，该怎么办呢？他们都扩展自 `Person`，但 `Person` 本身并没有这些子类型的所有链接和属性。所以下面这个查询并不会起作用：
...
输出的内容很多，所以这里只展示其中的一部分。你会注意到没有所需属性或链接的类型会返回一个空集：`{}`，比如“Woman 1”的 `age`：
...
看起来还不错，但输出没有告诉我们这些对象分别是什么类型。要在 EdgeDB 的查询中引用对象本身的类型，你可以使用 `__type__`。只调用 `__type__` 则只能得到一个 `uuid`，所以我们需要添加 `{name}` 来表明我们想要的是类型的名称。如果你想在查询中显示对象的类型，你可以访问这个字段，因为所有类型都有 `name` 这个字段。
...
上面的操作被称为 [多态查询（polymorphic query）](https://www.edgedb.com/docs/edgeql/overview/#ref-eql-polymorphic-queries)，这也是在我们的架构中使用抽象类型的理由之一.

## 超类型、子类型和泛型类型（Supertypes, subtypes, and generic types）

我们将那些被其他类型扩展的类型称为 `supertype`（超类型）。扩展出的类型是它们的 `subtypes`（子类型）。因为继承一个类型会得到它的所有特性，所以`subtype IS supertype` 将返回 `{true}`。当然，`supertype IS subtype` 将返回 `{false}`，因为超类型不继承其子类型的特性。

在我们的架构中，这意味着 `SELECT PC IS Person` 返回 `{true}`，而 `SELECT Person IS PC` 将返回 `{true}` 或 `{false}`，具体取决于所选对象是否是 `PC` .

想要通过查询进行确认，只需使用可计算数据（computable）`Person IS PC` 添加一个 [形状](https://www.edgedb.com/docs/edgeql/expressions/shapes/#ref-eql-expr-shapes) 查询（shape query），EdgeDB 则会告诉你：
...
幸运的是，`int16` 这些类型都是 [从抽象类型扩展](https://www.edgedb.com/docs/datamodel/abstract) 出来的，我们可以使用这些抽象类型，它们都以 `any` 开头，包括 `anytype`，`anyscalar`，`anyenum`，`anytuple`，`anyint`，`anyfloat`，`anyreal`。唯一可能让你不确定的是 `anyreal`：它意味着任何实数，包括整型和浮点型，以及 `decimal` 类型。

因此，你可以将上述输入简化为 `SELECT 1887 IS anyint` 并获得 `{true}`。

## Multi 的使用（Multi in other places）

我们已经看到过很多次 `multi link`，你可能想知道 `multi` 是否也可以用在其他地方。答案是肯定的。比如 `multi property`，它与任何其他属性一样，但它可以有多个值。例如，我们的 `Castle` 类型有一个用于 `doors` 属性的 `array<int16>`：
...
- `multi property` 与对象
  ...
  我们先来看使用 `multi property` 更好的两个方面，然后再说对象的好处。
  ...
  如果你需要每个对象具有多个值，则使用对象更容易进行迁移/变更（migrate）。

希望这些解释对你有所帮助。大多数时候，记住以上几点应该可以帮助你在众多选择中了解到自己真正的需要并做出更明智的决定。
